...
let notificationC = (req, res) => {
  ...
  if (user_notification_message === "") {
    res.send({
      successMessage: "Notification Message Can't Be Empty",
    ...
    connectionInfo.query(notificatinData, [value], (err, result, field) => {
      if (err) {
        // console.log(err)
        return res.send({
          successMessage: "Notification was not successfully uploaded",
          redirect: "/uploadNotification",
          message: "click here to Try Again",
        });
      } else {
        return res.send({
          successMessage: "Notification Uploaded successfully ",
          redirect: "/uploadNotification",
  ...
  connectionInfo.query(deleteNotification, (err) => {
    if (err) {
      ...
      return res.send({
        ...
        message: "Click Here To Go Back To Notification",
        redirect: "/uploadNotification",
      });
    }
  });
