...
let updateUserProfile = (req, res) => {
  ...
  try {
    ...
    let user_first_name_updated_trimmed = user_first_name
      .toLowerCase()
      .trim()
      .replace(/\s/g, "");
    let user_last_name_updated_trimmed = user_last_name
      .toLowerCase()
      .trim()
      .replace(/\s/g, "");
    let user_email_updated_trimmed = user_email
      .toLowerCase()
      .trim()
      .replace(/\s/g, "");
    let user_whatsapp_number_updated_trimmed = user_whatsapp_number
      .toLowerCase()
      .trim()
      .replace(/\s/g, "");
    let user_Indian_number_updated_trimmed = user_Indian_number
      .toLowerCase()
      .trim()
      .replace(/\s/g, "");
    let user_password_updated_trimmed = user_password.trim().replace(/\s/g, "");
    let user_study_section_updated_trimmed = user_study_section
      .toLowerCase()
      .trim()
      .replace(/\s/g, "");
    if (
      user_first_name_updated_trimmed === "" ||
      user_last_name_updated_trimmed === "" ||
      ...
      if (
        ...
        if (!emailValidator.test(user_email_updated_trimmed)) {
          ...
          if (
            ...
            !phoneNumberValidatorEthiopia.test(
              user_whatsapp_number_updated_trimmed
            )
          ) {
            return res.send({
